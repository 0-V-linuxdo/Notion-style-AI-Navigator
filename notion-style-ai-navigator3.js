...
(function (global) {
  ...
  class StorageManager {
    ...
    static getEffect() {
      try {
        return typeof GM_getValue !== 'undefined' ? GM_getValue(this.STORAGE_KEY, this.DEFAULT_EFFECT) : this.DEFAULT_EFFECT;
      } catch (e) {
  ...
  class PromptNavigator {
    ...
    #detectPlatform() {
      ...
      return this.platforms.find((p) => p.hosts.some((h) => currentHost.includes(h)));
    }
  ...
  global.NotionStyleAINavigator = {
    init: function(config) {
      new PromptNavigator(config).init();
    },
